...
function printBrand(painter, version, subtitle = 'By Roger Salinas · Dexly Studios') {
  ...
  console.log(
    `  ${painter.violet('│')}  ${painter.bold(painter.white(name))}${gap}` +
...
function printChoice(painter, key, title, detail) {
  console.log(
    `  ${painter.violet('[')}${painter.accent(key)}${painter.violet(']')} ` +
      `${painter.bold(title.padEnd(11, ' '))} ${painter.dim(detail)}`
  );
}
...
function printResultCard(painter, title, lines) {
  ...
  console.log(
    `  ${painter.success('│')}  ${painter.bold(title.slice(0, width - 2).padEnd(width - 2, ' '))}` +
      `${painter.success('│')}`
  );
